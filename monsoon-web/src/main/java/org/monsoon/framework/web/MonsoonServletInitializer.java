...
/**
 * This class implements the ServletContainerInitializer interface and is used
 * to initialize the web application.
 * It is annotated with @HandlesTypes to specify the types of classes that it
 * can handle.
 */
...
public class MonsoonServletInitializer implements ServletContainerInitializer {
     ...
     * context using Monsoon.run() and refreshes the context to get the
     * ServletWebAdapter.
     ...
     * @param servletContext the ServletContext that the ServletWebAdapter is
     *                       registered with
     * @throws ServletException if an error occurs while initializing the web
     *                          application
     */
    @Override
    public void onStartup(Set<Class<?>> set, ServletContext servletContext) throws ServletException {
        ...
        try {
            ...
            for (FilterRegistration filterRegistration : server.getDispatcher().getFilterRegistry()) {
                ...
                javax.servlet.FilterRegistration.Dynamic filterReg = servletContext.addFilter(filterName, filter);
            ...
            ServletRegistration.Dynamic servlet = servletContext.addServlet("dispatcher", server);
            servlet.addMapping("/");
            servlet.setLoadOnStartup(1);

            servletContext.addListener(new MonsoonServletContextListener());
